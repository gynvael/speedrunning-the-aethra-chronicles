...
Rules:
  ...
  4. DOSBox at constant 12000 cycles (`dosbox.conf` should have `cycles: #12000` in the `[cpu]` section). No changing of CPU cycles is allowed.
  ...
  6. Using F3 ([max sleep divisor](TODO)) is **allowed**. It can be done before you start the run.
  ...
  8. You must remain on the same version of the game and run only game (DOSBox) instance.
